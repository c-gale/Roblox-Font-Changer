# Roblox-Font-Changer (only-cursor so no fonts with this one)
...
To start, install the repository through .zip or clone it and then change your ```config.json``` to have the ```"robloxVersionsPath"``` field be your Roblox versions directory.
Save that and then drop the cursor you would like to use in the ```cursor``` directory and rename it to ```cursor.png``` (make sure you have the file extension as .png)
